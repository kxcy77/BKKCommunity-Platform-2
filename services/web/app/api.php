...
{
    $isDemo = strcasecmp((string) $row['category'], 'Demonstration') === 0
        || str_contains(strtolower((string) $row['title']), 'not a real event')
        || str_contains(strtolower((string) $row['description']), 'test content only')
        || str_contains(strtolower((string) $row['location']), 'do not travel');
    return [
        'id' => (int) $row['id'],
...
function api_set_attendance(int $eventId): never
{
    ...
    $db = api_database();
    $check = $db->prepare("SELECT e.id, c.name AS category, e.title, e.description, e.location
        FROM events e INNER JOIN event_categories c ON c.id = e.category_id
        WHERE e.id = ? AND e.status = 'published' AND e.end_at >= UTC_TIMESTAMP() LIMIT 1");
    $check->execute([$eventId]);
    $event = $check->fetch();
    if (!$event) {
        api_error(404, 'That upcoming event could not be found.', 'event_not_found');
    }
    if ($status === 'attending' && is_demonstration_event($event)) {
        api_error(422, 'This demonstration event cannot accept attendance.', 'demo_event');
    }
